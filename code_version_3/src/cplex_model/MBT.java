	...
	public void model_solver(Read_In ri )throws IOException{
		try {
						...
						for(int h = 0; h < A.length; h ++) {
							for(int g = 0; g < A[h].length; g ++) {
								if(A[h][g] != 0) {
									...
									IloNumExpr subexpr2 = this.cplex.numExpr();
									IloNumExpr subexpr3 = this.cplex.numExpr();
									subexpr1 = this.cplex.sum(this.ts[h][g][i][k], this.cplex.prod(this.ws[i][k], this.Delta1[i][k][p]));
									subexpr2 = this.cplex.diff(subexpr1, this.cplex.prod(this.M, this.cplex.diff(1, this.XS[h][g][i][k][p])));
									subexpr3 = this.cplex.sum(subexpr1, this.cplex.prod(this.M, this.cplex.diff(1, this.XS[h][g][i][k][p])));
									cplex.addLe(FS[h][g][i][k][p], subexpr3);
									cplex.addGe(FS[h][g][i][k][p], subexpr2);
								}
							}
						}
					}
			...
			for(int p : this.Ps) {
				...
				cplex.addLe(subexpr1, this.Z);
			}
			//---constraints(A.1n)-(A.1o)--//
			for(int p : this.Pm) {
				for(int k : this.K) {
					for(int h: this.K_0) {
						if(h != k) {
							...
							IloNumExpr subexpr2 = this.cplex.numExpr();
							IloNumExpr subexpr3 = this.cplex.numExpr();
							subexpr1 = this.cplex.sum(this.tm[h][k], this.Y[k][p]);
							subexpr2 = this.cplex.sum(subexpr1, this.cplex.prod(this.M, this.cplex.diff(1, this.XM[h][k][p])));
							subexpr3 = this.cplex.diff(subexpr1, this.cplex.prod(this.M, this.cplex.diff(1, this.XM[h][k][p])));
			...
			for(int i : this.N) {
				for(int k : this.K) {
					for(int p : this.Ps) {
						...
						cplex.addLe(this.Delta1[i][k][p], this.cplex.prod(this.M, subexpr1));
			...
			for(int k : this.K) {
				for(int p : this.Pm) {
					for(int i : this.N) {
						...
						cplex.addLe(this.Delta2[i][k][p], this.cplex.prod(this.M, subexpr1));
					}			
				}
			}
