		...
		try {
			...
			for(int p : this.Ps) {
				for(int k : this.K) {
					for(int i : this.N) {
						IloNumExpr subexpr1 = this.cplex.numExpr();
						subexpr1 = this.cplex.diff(1, 
									this.cplex.prod(this.M, this.mu1[i][k][p]));
						cplex.addGe(this.Delta1[i][k][p], subexpr1);
						IloNumExpr subexpr2 = this.cplex.numExpr();
						subexpr2 = this.cplex.prod(this.M, 
									this.cplex.diff(1, this.mu1[i][k][p]));
						cplex.addLe(this.Delta1[i][k][p], subexpr2);
						cplex.addEq(this.XS[i][k][p], this.cplex.diff(1, this.mu1[i][k][p]));
					}
				}
			}
			for(int p : this.Pm) {
				for(int k : this.K) {
					IloNumExpr subexpr1 = this.cplex.numExpr();
					for(int i : this.N) {
						subexpr1 = this.cplex.sum(subexpr1, this.Delta2[i][k][p]);
					}
					IloNumExpr subexpr2 = this.cplex.numExpr();
					subexpr2 = this.cplex.diff(1, 
								this.cplex.prod(this.M, this.mu2[k][p]));
					cplex.addGe(subexpr1, subexpr2);
					IloNumExpr subexpr3 = this.cplex.numExpr();
					subexpr3 = this.cplex.prod(this.M, 
								this.cplex.diff(1, this.mu2[k][p]));
					cplex.addLe(subexpr1, subexpr3);
					cplex.addEq(this.XM[k][p], this.cplex.diff(1, this.mu2[k][p]));
				}
			}
